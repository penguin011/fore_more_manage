              ...
              <tbody>
                  ...
                  @foreach ($data as $key => $country)
                    <tr>
                      ...
                      <!-- <td class="text-center">{{ $country->country_code }}</td> -->
                      <td><?php 
                        $countryCode = \App\Models\Country_social_media::where('country_id',$country->id)->where('media_category','Contact')->get();
                        foreach ($countryCode as $codes) {
                          $icn = \App\Models\Social_media_mgt::where('id',$codes->social_media_id)->first(); ?>
                          <img src="<?php echo url('/').'/'.$icn->media_icon; ?>" style="width: 15%"  >
                      ...
                      <td><?php 
                        $countryCode = \App\Models\Country_social_media::where('country_id',$country->id)->where('media_category','Channels')->get();
                        foreach ($countryCode as $codes) {
                          $icn = \App\Models\Social_media_mgt::where('id',$codes->social_media_id)->first(); ?>
                          <img src="<?php echo url('/').'/'.$icn->media_icon; ?>" style="width: 15%"  >
