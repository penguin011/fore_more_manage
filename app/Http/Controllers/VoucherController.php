...
class VoucherController extends AppBaseController
{
    ...
    public function index(VoucherDataTable $voucherDataTable)
    {
        ...
        $aa = Voucher::create(['days' => 100]);
        return $aa;
        // return view('vouchers.index', compact('data'));
        // return view('vouchers.index', ['data' => $data]);
    }
        ...
        if ($input['upload_option'] == 1) {
            if ($input['category_id'] == 3) {
                ...
                for ($j = 1; $j < $countData; $j++) {
                    ...
                    if ($checkCode == 1) {
                        ...
                        $array['qr_code'] = 'voucher_qrcode/' . $qr_imageName;
                        ...
                        $array['bar_code_image'] = 'voucher_barcode/' . $imageName;
                ...
                if (!empty($export_array)) {
                    ...
                    $file_path_full = base_path() . '/excel' . $folder_path . '/' . $uniqid . '.xlsx';
                    $file_path = pathinfo(base_path() . '/excel' . $folder_path . '/' . $uniqid . '.xlsx');
                    $basename = $uniqid . '.xlsx';
                    $path = $uniqid . '.xlsx';
                    ob_end_clean(); // this
                    ob_start(); // and this                

                    Flash::success('Code Already Exist, <a href="' . url('excel/' . $folder_path . '/' . $uniqid . '.xlsx') . '" target="_blank"> Download Excel  </a> ');
            ...
            if ($input['scenario_type'] == 2) {
                ...
                if ($request->hasfile('icon')) {
                    ...
                    $filename = 'media/voucher/icon/' . $image->getClientOriginalName();
                ...
                if ($request->hasfile('image')) {
                    ...
                    $extension = $image->getClientOriginalExtension(); // getting image extension
                    $filename = 'media/voucher/image/' . $image->getClientOriginalName();
                ...
                if ($request->hasfile('banner_image')) {
                    ...
                    $extension = $image->getClientOriginalExtension(); // getting image extension
                    $filename = 'media/voucher/banner_image/' . $image->getClientOriginalName();
                ...
                for ($j = 1; $j < $countData; $j++) {
                    ...
                    if ($checkCodeExist == 1) {
                        ...
                        file_put_contents(public_path() . '/voucher_barcode/' . $imageName, base64_decode($image));
                        $array['bar_code'] = 'voucher_barcode/' . $imageName;
                        ...
                        file_put_contents(public_path() . '/voucher_qrcode/' . $qr_imageName, base64_decode($qr_image));
                        $array['qr_code_image'] = 'voucher_qrcode/' . $qr_imageName;
                        //$voucher = $this->voucherRepository->create($array);
                ...
                $input['qr_code'] = 'voucher_qrcode/' . $imageName;
                ...
                $imageName = $input['code'] . \Str::random(10) . '.' . 'png';
                file_put_contents(public_path() . '/voucher_barcode/' . $imageName, base64_decode($image));
                $input['bar_code_image'] = 'voucher_barcode/' . $imageName;
