...
class Social_media_mgtAPIController extends AppBaseController
{
    ...
    public function get_social_media_link($id)
    {
        $channels = \App\Models\Country_social_media::leftjoin('social_media_mgt','country_social_media.social_media_id','social_media_mgt.id')
            ->where('country_social_media.country_id',$id)
            ->where('country_social_media.media_category','Channels')
            ->select("country_social_media.*","social_media_mgt.media_icon")
            ->get();

        $contact = \App\Models\Country_social_media::leftjoin('social_media_mgt','country_social_media.social_media_id','social_media_mgt.id')
            ->where('country_social_media.country_id',$id)
            ->where('country_social_media.media_category','Contact')
            ->select("country_social_media.*","social_media_mgt.media_icon")
            ->get();


        return response(['status'=>'200','Message'=>'data retrieved successfully.','channels' => $channels,"contact" =>$contact]);
    }
    /**
