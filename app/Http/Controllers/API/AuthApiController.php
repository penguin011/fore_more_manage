...
class AuthApiController extends Controller
{
        ...
        if (!empty($user_no) || !empty($user_email)) {
            ...
            curl_setopt_array(
                ...
                array(
                    ...
                    CURLOPT_POSTFIELDS => "{ \"body\": \"for-more.eu code is: " . $request->otp . "\",\"to\" : \"" . $request->mobile_code . $request->mobile_no . "\",\"from\": \"for-more.eu\"}",
    ...
    public function register(Request $request)
    {
        ...
        file_put_contents('/users/' . $imageName, base64_decode($image));
        $input['bar_code'] = 'users/' . $imageName;
        ...
        file_put_contents('/users_qr/' . $qr_imageName, base64_decode($qr_image));
        $input['qr_code'] = 'users_qr/' . $qr_imageName;
    ...
    public function delete_user_account($id)
    {
        ...
        $userUpdate = User::where('id', $id)->update($data, $id);

        if ($userUpdate != '') {
            $getUser = User::leftJoin('country', 'users.residence_country_id', 'country.id')->where('users.id', $id)->select('users.*', 'country.country_name as countryName')->first();

            $userData['unique_no'] = $getUser['unique_no'];
            $userData['name'] = $getUser['name'];
            $userData['country'] = $getUser['countryName'];

            $response = Email_master::send_delete_acc_mail($userData);


            return response(['status' => '200', 'Message' => 'Account deleted successfully.']);
        } else {
            return response(['status' => '401', 'Message' => "Something went wrong please try again."]);
        }
    ...
    public function login(Request $request)
    {
        ...
        if ($user['status'] == '1') {
            ...
            return response(['status' => '401', 'Message' => "This profile has been deleted."]);
        }
    ...
    public function forget_password(Request $request)
    {
        ...
        // });
        // return $resetLink;
        // Mail::send('emails.forgetPassword', ['resetLink' => $resetLink], function ($message) use ($request) {
        //     $message->to($request->email);
        //     $message->subject('Reset Password');
        // });
        // Mail::send()
