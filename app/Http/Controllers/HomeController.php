...
class HomeController extends Controller
{
    ...
    public function buss_mediaList(Request $request)
    {  
        $mediaList = \App\Models\Social_media_mgt::where('media_category',$request->category_id)
                                            ->pluck('media_name','id');
    ...
    public function business_user_performance_export($start_date,$end_date)
    {
        $transactionArray = array();
            if(isset($start_date) && $start_date != '' && isset($end_date) && $end_date != '') 
            {
                ...
                $business_id  = \App\User::where('id',$buss_id->business_id)->first(); 


                $business_details = \App\Models\Brand::where('id',$business_id->userDetailsId)->first();

                $rewardsStamp = \App\Models\My_rewards::where('buss_id',$business_id->userDetailsId)
                        ->whereDate('created_at','>=', $fromDate)
                        ->whereDate('created_at','<=', $toDate)
                        ->selectRaw('sum(stamps) as totalStamp')->first();


                $rewardsPoint = \App\Models\My_rewards::where('buss_id',$business_id->userDetailsId)
                        ->whereDate('created_at','>=', $fromDate)
                        ->whereDate('created_at','<=', $toDate)
                        ->selectRaw('sum(point_per_stamp) as totalPoint')->first();
            
                $freeVoucher = \App\Models\Voucher::where('business_id',$business_id->userDetailsId)
                                ->whereDate('created_at','>=', $fromDate)
                                ->whereDate('created_at','<=', $toDate)
                                ->where('category_id',1)
                                ->selectRaw('count(id) as totalFreeVoucher')->first();

                $superDeal = \App\Models\Voucher::where('business_id',$business_id->userDetailsId)
                                ->whereDate('created_at','>=', $fromDate)
                                ->whereDate('created_at','<=', $toDate)
                                ->where('category_id',2)
                                ->selectRaw('count(id) as totalSuperDeal')->first();

                $superCode = \App\Models\Voucher::where('business_id',$business_id->userDetailsId)
                                ->whereDate('created_at','>=', $fromDate)
                                ->whereDate('created_at','<=', $toDate)
                                ->where('category_id',3)
                                ->selectRaw('count(id) as totalSuperCode')->first();
                ...
                $birthdayCount = \App\Models\Transaction_history::where('buss_id',$business_id->userDetailsId)
                                ->whereDate('created_at','>=', $fromDate)
                                ->whereDate('created_at','<=', $toDate)
                                ->where('transaction_type_id',2)
                                ->selectRaw('sum(stamps) as totalBirthday')->first();

                $welcomeCount = \App\Models\Transaction_history::where('buss_id',$business_id->userDetailsId)
                                ->whereDate('created_at','>=', $fromDate)
                                ->whereDate('created_at','<=', $toDate)
                                ->where('transaction_type_id',6)
                                ->selectRaw('sum(stamps) as totalWelcome')->first();
    ...
    public function business_user_performance(Request $request)
    {
        $buss_id = \App\User::where('id',Auth::user()->id)->first();
        $business_id  = \App\User::where('id',$buss_id->business_id)->first(); 
        ...
        return view('report.business_user_performance',compact('rewardsStamp','rewardsPoint','freeVoucher','superDeal','superCode','birthdayCount','welcomeCount','business_details','fromDate','toDate'));
    }

    public function business_user(Request $request)
    {   
        if(isset($request->from_date) && $request->from_date != '' && isset($request->to_date) && $request->to_date != ''&& isset($request->user_id) && $request->user_id != '')
        {
            $fromDate = $request->from_date;
            ...
            $voucher_user = \App\Models\Voucher_upload_receipt::where('business_id',$business_id->userDetailsId)
                            ->where('user_id', $request->user_id)
                            ->whereDate('created_at','<=', $toDate)
                            ->whereDate('created_at','<=', $toDate)
                            ->select('user_id')
            ...
            $stamp_user = \App\Models\Transaction_history::where('buss_id',$business_id->userDetailsId)
                            ->where('user_id', $request->user_id)
                            ->whereDate('created_at','>=', $fromDate)
                            ->whereDate('created_at','<=', $toDate)
                            ->select('user_id')
        ...
        $voucher_user_dropdown = \App\Models\Voucher_upload_receipt::where('business_id',$business_id->userDetailsId)
                            ->select('user_id')
                            ->groupBy('user_id')
                            ->get()->toArray();
            $stamp_user_dropdown = \App\Models\Transaction_history::where('buss_id',$business_id->userDetailsId)
                            ->select('user_id')
                            ->groupBy('user_id')
                            ->get()->toArray();
        ...
        return view('report.business_user',compact('userData','userData_dropdown','user_id','business_details','fromDate','toDate'));
    }
    public function business_user_order(Request $request)
    {   
        $buss_id = \App\User::where('id',Auth::user()->id)->first();
        $business_id  = \App\User::where('business_id',$buss_id->business_id)->where('role_id','5')->where('email',$buss_id->email)->first(); 
        ...
        if(isset($request->from_date) && $request->from_date != '' && isset($request->to_date) && $request->to_date != ''&& isset($request->status) && $request->status != '')
        {
            ...
            $orders_data = \App\Models\Member_orders::join('users','member_orders.member_id','users.id')
                    ...
                    ->whereDate('member_orders.created_at','<=', $toDate)
                    ->where('member_orders.status', $status)
                    ->where('member_orders.created_by',$business_id->business_id)
                    ->select('member_orders.*','users.name','users.unique_no')
                    ->get();
        }
        else if(isset($request->from_date) && $request->from_date != '' && isset($request->to_date) && $request->to_date != '')
        ...
        $bussData = User::where('id',$buss_id->business_id)->first();
        
        $business_details = \App\Models\Brand::where('id',$bussData->userDetailsId)->first();

        return view('report.business_user_order',compact('orders_data','business_details','fromDate','toDate','status'));
    }
    public function business_user_appointment(Request $request)
    {
        $buss_id = \App\User::where('id',Auth::user()->id)->first();
        $business_id  = \App\User::where('business_id',$buss_id->business_id)->where('role_id','5')->where('email',$buss_id->email)->first(); 
        ...
        if(isset($request->from_date) && $request->from_date != '' && isset($request->to_date) && $request->to_date != ''&& isset($request->status) && $request->status != '')
        {
            ...
            $appointment_data = \App\Models\Booked_services::join('users','booked_services.member_id','users.id')
                ->whereDate('booked_services.created_at','>=', $fromDate)
                ->whereDate('booked_services.created_at','<=', $toDate)
                ->where('booked_services.created_by',$business_id->business_id)
                ->where('booked_services.status',$status)
                ->select('booked_services.*','users.name','users.unique_no')
                ->get();
        ...
        return view('report.business_user_appointment',compact('appointment_data','business_details','fromDate','toDate','status'));
    }
}
